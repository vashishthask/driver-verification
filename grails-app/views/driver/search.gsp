	...
	<body>
		...
		<div class="nav" role="navigation">
			<ul>
				...
				<sec:ifAnyGranted roles="ROLE_USER,ROLE_VERIFIER">
		...
		<div id="list-driver" class="content scaffold-list" role="main">
			<h1><g:message code="default.list.search.label" args="[entityName]" /></h1>
			...
			<thead>
					<tr>
					    ...
					    <g:sortableColumn property="status" title="${message(code: 'driver.status.label', default: 'Status')}" />
				...
				<g:each in="${driverInstanceList}" status="i" var="driverInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: driverInstance, field: "status")}</td>
		...
		<filterpane:filterPane domain="com.amigo.verifydriver.Driver" 
			filterProperties="name,mobileNumber,drivingLicenseNumber,voterIdProof,aadharIdProof,status"
			action="search" visible="true" title="Search Driver"/>
	</body>
</html>
